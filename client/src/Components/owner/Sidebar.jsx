  ...
  return (
    ...
    w-full border-r border-borderColor text-sm'>
      ...
      {image && (
        ...
        text-primary cursor-pointer'>Save <img src={assets.check_icon} width={13} alt=""  /></button>
      )}
      ...
      <div className='w-full'>
        {ownerMenuLinks.map((link,index)=>(
           <NavLink key={index} to={link.path} className={`relative flex items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path===location.pathname ? 'bg-primary/10 text-primary' : 'text-gray-500'}`}>
